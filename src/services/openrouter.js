...
- Pay close attention to MOVEMENT:
  ...
  * Prefer the ULTIMATE / most significant target — a major city or an OBLAST — over small intermediate waypoint villages. If a post lists waypoints then a bigger target (e.g. "в сторону Острой Луки, Троица. Возможно на северо-восток в сторону Нижегородской области"), set destination to the major one ("Nizhny Novgorod Oblast"), NOT the village ("Troitsa").
  * "heading" = the compass direction ONLY. Must be EXACTLY one of: "north","north-east","east","south-east","south","south-west","west","north-west". Derive it from explicit direction words ("на северо-восток"→"north-east", "север"→"north", "восток"→"east", etc.) or from knowing which direction the destination is. NEVER put place names in heading. If you cannot determine a compass direction with confidence, set heading to null.
  * Examples: "в направлении Москвы" from Тула → destination="Moscow", heading="north". "курс на восток" → heading="east". "на северо-восток в сторону Нижегородской области" → destination="Nizhny Novgorod Oblast", heading="north-east".
